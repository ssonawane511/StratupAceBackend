...
import "./startupMessages.css";

import MentorRecentMessage from "../../components/MentorRecentMessage";
...
const MentorMessgaes = () => {
  ...
  useEffect(() => {
    ...
    setInterval(() => {
      Axios({
        method: "get",
        url: `/api/startups/get_messages/${id}`,
      }).then((res) => {
        if (res.data.success) {
  ...
  useEffect(() => {
    if (user) {
      Axios({
        method: "get",
        url: `/api/startups/get_converstaions/${user.uid}`,
      }).then((res) => {
        if (res.data.success) {
  ...
  const handleSendMessage = (e) => {
    ...
    Axios({
      method: "post",
      url: `/api/startups/send_messages/${id}`,
      data: {
        message: text,
      },
    }).then((res) => {
            ...
            <div className='col  no-margin p-25 shadow-b border-l'>
              ...
              <p>
                ...
                <sup className='ml-10 highligh'>
                  <Link
                    to={`/mentor/profile/${selectedConversation.mentor.handle}`}>
                    @{selectedConversation.mentor.handle}
                  </Link>
                </sup>
              </p>
            </div>
            ...
            <div className='left-chat-container'>
              ...
              {conversations.length > 0
                ? conversations.map((convo) => (
                    <MentorRecentMessage data={convo} />
                  ))
                : ""}
            </div>
          </div>
